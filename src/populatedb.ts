...
import { Genre, IGenreModel } from "./models/genre";
...
type BookInstanceType = mongoose.Document<
  unknown,
  {},
  IBookInstanceModel
> &
...
type BookType = mongoose.Document<unknown, {}, IBookModel> &
  Omit<
    IBookModel & {
      _id: mongoose.Types.ObjectId;
    },
...
type AuthorType = mongoose.Document<unknown, {}, IAuthorModel> &
  Omit<
    IAuthorModel & {
      _id: mongoose.Types.ObjectId;
    },
...
// Gets connection string from command line
const userArgs = process.argv.slice(2);

// Define Arrays with types
let books: BookType[] = [];
let genres: GenreType[] = [];
let authors: AuthorType[] = [];
let bookinstances: BookInstanceType[] = [];
...
// Connects to MongoDb and create collections with instances
...
async function main() {
  ...
  // Creates collections and instances
  await createGenres();
  await createAuthors();
  await createBooks();
...
async function bookInstanceCreate(
  ...
  const bookinstancedetail: IBookInstanceDetail = {
    book,
    imprint,
  };
